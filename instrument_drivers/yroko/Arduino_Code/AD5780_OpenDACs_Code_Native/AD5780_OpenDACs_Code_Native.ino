...
void normalMode()
{
  ...
  for( int i = 0; i <= 3; i++)
  {
    ...
    if (attemps>=10)
    {
      SerialUSB.print("ERROR INITIALIZING DAC");
      SerialUSB.println(i);
    }
    else if (o!=2)
    {
...
void ID()
{
  SerialUSB.println("DCBOX_QUAD_AD5780");
}
...
void loop() 
{
  ...
  if(initialized == 0)
  {
    ...
    for( int i = 0; i <= 3; i++)
    {
      float v = writeDAC(i,0); /* set all channels to 0 on startup*/
    }
