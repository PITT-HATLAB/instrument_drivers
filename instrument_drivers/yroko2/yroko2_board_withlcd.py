...
class Yroko2Board:
    ...
    def __init__(self):
        ...
        self.channel_dict = {0: "GPIO6", 1: "GPIO5"}  # , 2: "GPIO16"}

        # Will connect to LCD display below. address can be found out by using 'i2cdetect -y 1'
        self.lcd = CharLCD(i2c_expander='PCF8574', address=0x27, port=1, charmap='A02', auto_linebreaks=True, backlight_enabled=True)
        self.lcd_spinner = cycle(['.  ','.. ','...',' ..','  .','   '])
    ...
    def __enter__(self):
        ...
        self.spi.max_speed_hz = int(35E6)#int(27777777/100000)
    ...
    def setup_lcd(self):
        ...
        return
    def update_LCD(self,channel_being_ramped : int, up_down : str):
        """
        ...
        if channel_being_ramped == 0:
            ...
            ch1_status = (f"RAMPING {up_down}" + next(self.lcd_spinner)) if channel_being_ramped == 0 else "IDLE"
            self.lcd.write_string(f"{ch1_status}\n\r")
        if channel_being_ramped ==1:
            self.lcd.cursor_pos = (2,5)
            self.lcd.write_string(f"{yk._bytesToVoltage(yk._getDACValue(1)):13.5f}")
...
if __name__ == "__main__":
    ...
    with yroko as yk:
        ...
        # ramp down
        print("RAMPING 0")
        yk.ramp(channel=0, start_voltage=1, stop_voltage=-1)
        print("done")
        yk.setup_lcd()
    ...
    with yroko as yk:
        ...
        while True:
            ...
            try:
                ...
                yroko.lcd.write_string('Accepting Connections.\n\r')
                connection = None
